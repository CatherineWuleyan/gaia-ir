...
def verbose_validate(raw_answer: str, original_content: str) -> None:
    ...
    """
    fenced_content = lv._strip_markdown_fence(raw_answer)
    candidates = [("原样", raw_answer)]
    if fenced_content is not None:
        candidates.append(("剥离markdown代码块围栏后", fenced_content))

    parsed = None
    used_desc = None
    align_failed_all = True
    for desc, text in candidates:
        aligned = lv._align_text_fields(text, original_content)
        if aligned is None:
            continue
        align_failed_all = False
        repaired = lv._replace_text_field_values(text, aligned)
        pool = "".join(decoded_text for decoded_text, _, _ in aligned)
        repaired = lv._escape_remaining_invalid_backslashes(repaired, pool)
        try:
    ...
    if parsed is None:
        if align_failed_all:
            print("[诊断] \"text\"字段跟原文逐字符对齐失败——不管原样还是剥掉markdown围栏,")
            print("        都没能把所有text字段内容对齐到original_content上(不是空白差异、")
            print("        也不是转义歧义能解释的不一致)。这通常意味着模型输出的内容本身")
            print("        跟原文有实质性出入(漏字/多字/改写),不只是转义符号的问题。")
        else:
            print("[诊断] text字段对齐成功,但把对齐结果替换回去之后,JSON仍然解析失败")
            print("        (大概率是of_terms等其它字段里还有转义问题)。")
        print("------ 原始返回内容 ------")
        print(raw_answer)
        return

    if used_desc != "原样":
        print(f"[诊断] 原始返回不是直接能解析的JSON,用了这个方式才解析成功: {used_desc}")
    print("[诊断] \"text\"字段跟原文的逐字符对齐通过——内容匹配、反斜杠转义判断都没问题。")
    ...
    for idx, (lbl, info) in enumerate(zip(labels, extracted)):
        if lbl == "instance" or (lbl == "elaboration" and "of_terms" in info):
            self_only = lv._check_terms_grounding(info["of_terms"], idx, segments)
            if self_only is None:
                print(f"  segment[{idx}]: label={lbl!r} 的 of_terms={info['of_terms']} 哪儿都找不到(自己这段没有、原文别处也没有) -> 校验失败(幻觉短语)")
                return
            if self_only:
                print(f"  segment[{idx}]: label={lbl!r} 的 of_terms 里 {self_only} 只在自己这段里找到,原文别处没有印证(不算失败,会带警告字段)")
