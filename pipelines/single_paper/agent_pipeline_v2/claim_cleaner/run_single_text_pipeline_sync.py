...
if __package__:
    from .build_single_text_graph import build_graph_for_text
    from . import run_full_pipeline as sync_pipeline
else:  # pragma: no cover - retained for direct command-line use
...
def _write_note_only_result(paper_dir: Path, text: str) -> bool:
    """Emit the established claims_final shape when Step 1 found no claims.

    A standalone background condition is a valid note.  It must not be
    rejected merely because the claim-oriented fourteen-step path has no
    records to process.
    """
    organized = json.loads((paper_dir / "organized_content.json").read_text(encoding="utf-8"))
    claim_labels = {"assertion", "论据", "example"}
    if any(part["label"] in claim_labels for item in organized for part in item["organized_parts"]):
        return False
    conclusion = organized[0]["id"].split("::")[-1]
    (paper_dir / "claims_final.json").write_text(
        json.dumps({"claim": [], "note": [{"conclusion": conclusion, "text": text, "number": 1}], "relation": []},
...
def run_single_text(text: str, *, raise_on_failure: bool = False) -> str:
    ...
    for i, (script_rel_path, expected_output, needs_api) in enumerate(sync_pipeline.STEPS, start=1):
        ...
        if not ok:
            ...
            return paper_id
        if script_rel_path == "step1_process_conclusions/step1b_organize_labeled_content.py" and _write_note_only_result(paper_dir, text):
            print("未提取到claim；已将输入保留为一条note并写入 claims_final.json")
            return paper_id
