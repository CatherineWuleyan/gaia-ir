#!/usr/bin/env bash
# Job-side runner: execute ONE single-paper V2 run for a minibatch test on Bohrium.
#
# Usage (inside the Bohrium job container, cwd = uploaded input directory root):
#   bash job_run.sh test3
#
# Layout expected at the staging root (see stage_and_submit.sh):
#   agent/                       pipeline_harness kernel + .env with DeepSeek settings
#   pipelines/single_paper/      agent_pipeline_v2 pipeline code
#   data/minibatch/<paper>/      ocr.md + claims_final.json
#   manifests/<paper>.json       input manifest (relative paths)
#   PROVENANCE.txt               local git commit / dirty flag / file hashes
#
# Everything written under results/ is the only thing Bohrium uploads back
# (backward_files = ["results/"]), so the DeepSeek key in agent/.env is never
# copied to the personal disk.
set -uo pipefail

PAPER="${1:?usage: job_run.sh <paper>}"
...
say "paper=$PAPER host=$(hostname) pwd=$ROOT"
...
# does not ("sk-...: command not found").  Parse it the same tolerant way.
load_env_file() {
...
say "DEEPSEEK_BASE_URL=${DEEPSEEK_BASE_URL:-<unset>} DEEPSEEK_MODEL=${DEEPSEEK_MODEL:-<unset>}"

export PYTHONPATH="$ROOT/agent:$ROOT/pipelines/single_paper:$ROOT"
...
# wheels, and `pip install -e .` additionally needs PEP 517 build isolation
# (setuptools) - so install the published gaia-lang wheel instead of building the
# local harness (pipeline_harness already resolves from PYTHONPATH).
# Pin the local gaia-a7 versions so the remote compiler sees the same numerics.
PINS=(
...
# Environment-only check: `bash job_run.sh smoke` stops here.
if [[ "$PAPER" == "smoke" ]]; then
...
# ------------------------------------------------------------------ the run
CONFIG="$ROOT/pipelines/single_paper/pipeline.step1-5.json"
MANIFEST="$ROOT/manifests/$PAPER.json"
...
run_dir="$(python3 -m pipeline_harness init \
  --pipeline "$CONFIG" --inputs "$MANIFEST" --runs-root "$RUNS_ROOT" 2>>"$LOGS/$PAPER.init.log")"
if [[ -z "$run_dir" ]]; then
  say "FATAL init failed"
  tail -20 "$LOGS/$PAPER.init.log"
  printf '%s\t%s\t%s\n' "$PAPER" 6 "init_failed" >> "$RESULTS/status.tsv"
  exit 6
fi
say "run_dir=$run_dir"
...
python3 -m pipeline_harness run --run "$run_dir" \
  > "$LOGS/$PAPER.run.json" 2> "$LOGS/$PAPER.stderr.log"
status=$?
kill "$reporter" 2>/dev/null
trap - EXIT

elapsed=$(( $(date +%s) - started ))
printf '%s\t%s\t%s\t%ss\t%s\n' "$PAPER" "$status" "$(date -u +%FT%TZ)" "$elapsed" "$run_dir" \
  >> "$RESULTS/status.tsv"
say "finished paper=$PAPER exit=$status elapsed=${elapsed}s"
